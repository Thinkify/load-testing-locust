...
def generate_dashboard(jenkins_logs):
    ...
    div_for_throughput = util.FiguresGenerator().create_table_from_csv(
        'responses_requests')
    div_for_distribution = util.FiguresGenerator().create_table_from_csv(
        'responses_distribution')
    util.RequestDescriptor().generate_logs_from_existing_jenkins_logs(
        jenkins_logs)
